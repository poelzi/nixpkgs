...
let
  ...
  # These depend only on the package sets, not on the chain link/version, so
  # compute them once and share across all links instead of recomputing the
  # `.override` inside every `mkLink` (was 4 calls × every link). Keeps the
  # per-link `import ./default.nix` arguments byte-identical.
  llvmSharedForBuild = llvmSharedFor pkgsBuildBuild;
  llvmSharedForHost = llvmSharedFor pkgsBuildHost;
  llvmSharedForTarget = llvmSharedFor pkgsBuildTarget;
  llvmShared = llvmSharedFor pkgsHostTarget;

  # Also link-invariant: the extra positional args forwarded to ./default.nix.
